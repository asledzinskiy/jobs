node('calico'){
  ...
  try {
    ...
    dir("${WORKSPACE}/bird_repo"){
      ...
      stage('Publishing bird artifacts'){
        dir("artifacts"){
          ...
          def gitCommit = sh(returnStdout: true, script: "git -C ${WORKSPACE}/bird_repo rev-parse --short HEAD").trim()
    ...
    def BIRD_BUILD="${ARTIFACTORY_URL}/${GERRIT_CHANGE_NUMBER}/calico-bird/lastbuild".toURL().text.trim()
    def BIRD_URL="${ARTIFACTORY_URL}/${GERRIT_CHANGE_NUMBER}/calico-bird/bird-${BIRD_BUILD}"
