    ...
    try {
      ...
      dir("${env.WORKSPACE}/tmp_libcalico"){
        ...
        stage ('Build calico/build image') {
          docker.setDockerfileLabels("./Dockerfile", ["docker.imgTag=${buildImgTag}"])

          sh """
                make calico/build
                docker tag calico/build ${buildImg}:${buildImgTag}
             """
        }
...
def promote_artifacts () {
    node('calico') {
        stage('promote') {
          ...
          def felixArtifactUri = artifactory.uriByProperties(artifactoryServer.getUrl(), properties)
